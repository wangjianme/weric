...
<script>
  ...
  function requestNewPage(){
  	...
  	window.location.href="reb!queryMyRebursementCheck.action?curPage="+curPage
  }
  ...
  function openDialog(rebId){
	if (confirm("需要填写回退说明?")){
		...
		if(returnv != undefined){
			window.location.href="reb!back.action?rebId="+rebId+"&message="+returnv+"&flag=2&backPath=check";
		}
	}else{
      window.location.href="reb!updateFlag.action?flag=2&rebId="+rebId+"&backPath=check";
	}
	return false;
  }
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="main_table02" >
  <tr>
    <td align="left" width="25%"><table width="167" border="0" cellpadding="0" cellspacing="0">
      <tr>
        <td align="left" class="back_main04" ><span class="font_bold">财务报销单管理页面</span></td>
      </tr>
    </table></td>
    <td width="45%" align="left">
    <form name="f2" method="post" action="reb!queryCheckByRebNumber.action" >
    ...
    <td colspan="3"> 
         ...
         <c:forEach items="${requestScope.pageModel.list}" var="reb">
           <tr>
            ...
            <td align="center" class="back_main03"><span class="font_bold"><c:if test="${reb.flag ==1}">审核中</c:if><c:if test="${reb.flag ==3}"><a href='' class="link_blue" onclick="return showMessage('${reb.rebId}');">回退还审</c:if></a><c:if test="${reb.flag ==4}">审核完成</c:if></span></td>       
            <td align="center" class="back_main03">
              【<a href='reb!queryOne.action?rebId=${reb.rebId }&oper=disabled&check=check' class="link_blue">查看</a>】
               <c:if test="${reb.flag == 1 || reb.flag == 3}">
               【<a href='reb!updateFlag.action?flag=2&rebId=${reb.rebId }&backPath=check' class="link_blue" onclick="return openDialog('${reb.rebId }')">回退</a>】
               【<a href='reb!updateFlag.action?flag=4&rebId=${reb.rebId }&backPath=check' class="link_blue" onclick="return confirm('此操作将会对该报销单产生审核通过效果，确认操作吗?')">审核通过</a>】
               </c:if>
               <c:if test="${reb.flag == 4}">
                【<a href='reb!queryOne.action?rebId=${reb.rebId }&path=print&type=reb' target="_blank" class="link_blue">打印报销单</a>】
                【<a href='reb!queryOne.action?rebId=${reb.rebId }&path=print&type=mx' target="_blank" class="link_blue">打印明细单</a>】
                【<a href='reb!updateFlag.action?flag=1&rebId=${reb.rebId }&backPath=check' class="link_blue" onclick="return confirm('此操作将会对该报销单产生审重审，确认操作吗?')">重审</a>】
               </c:if>
             </td>
          </tr>
         </c:forEach>
  ...
  <tr>
    <td height="50" colspan="3" align="center">
       ...
       <c:if test="${requestScope.pageModel.count > 6}">
          <a href="reb!queryMyRebursementCheck.action?curPage=<c:if test="${param.curPage !=1}">${param.curPage-1 }</c:if><c:if test="${param.curPage ==1}">${param.curPage }</c:if>">上一页</a>|
          <a href="reb!queryMyRebursementCheck.action?curPage=<c:if test="${param.curPage != requestScope.pageModel.totalPage}">${param.curPage+1 }</c:if><c:if test="${param.curPage ==requestScope.pageModel.totalPage}">${requestScope.pageModel.totalPage }</c:if>">下一页</a>
       </c:if>
      <c:if test="${requestScope.pageModel.count <= 6}">
          上一页|下一页
       </c:if>
