...
<html>
  ...
  <script language="javascript">
  	 Ext.onReady(function(){
		var cate = '<c:out value="${eq.eqCate}"/>';
		...
		var viewForm = new Ext.form.FormPanel({
			...
			items:[
				...
				},
				{
					...
					cls:'tt',
					id:'quesScore1',
					value:'<c:out value="${eq.eqScore}"/>分'
				},
				{
					xtype:'panel',
					fieldLabel:'题干',
					...
					autoScroll:true,
					cls:'tt',
					html:'<c:out value="${eq.eqBody}" escapeXml="false"/>'
				},
				{
					xtype:'hidden',
					fieldLabel:'得分',
			...
			buttons:[
				{
					...
					handler:function(){
						makeScore();
						var score = Ext.getCmp('quesScore2').getValue();
			...
			listeners:{
				beforeshow:function(){
					...
					if(cate==2){		//判断题
						...
						var vals = '<c:out value="${eq.eqChoose}"/>';
						var vvs = vals.split("###");
						var array = new Array();
